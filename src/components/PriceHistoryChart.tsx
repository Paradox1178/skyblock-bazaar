...
import { getPriceHistory, ApiPriceHistory } from '@/api/client';
...
const PriceHistoryChart = ({ itemId }: PriceHistoryChartProps) => {
  const [history, setHistory] = useState<ApiPriceHistory[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    getPriceHistory(itemId)
      .then(data => setHistory(data.reverse())) // oldest first
      .catch(() => setHistory([]))
  ...
  if (loading) {
    return (
      <div className="bg-[#2a2a2a] border-2 border-[#1e1e1e] p-8 text-center">
        <p className="text-gray-500 text-sm font-bold animate-pulse">Lade Preishistorie...</p>
      </div>
    );
  }
  ...
  const maxPrice = Math.max(...history.map(e => e.high_price));
  const minPrice = Math.min(...history.map(e => e.low_price));
  const range = maxPrice - minPrice || 1;

  const lastEntry = history[history.length - 1];
  const prevEntry = history.length > 1 ? history[history.length - 2] : null;
  const trend = prevEntry ? lastEntry.avg_price - prevEntry.avg_price : 0;
  ...
  return (
    <div className="bg-[#2a2a2a] border-2 border-[#1e1e1e] p-6">
      ...
      <div className="flex items-end gap-1 h-32 mb-4">
        {history.map((entry) => {
          const height = ((entry.avg_price - minPrice) / range) * 100;
          ...
          return (
            <div key={date} className="flex-1 flex flex-col items-center group relative">
              <div className="absolute bottom-full mb-2 hidden group-hover:block bg-black/90 border border-[#333] p-2 text-[10px] whitespace-nowrap z-10">
                ...
                <p className="text-yellow-500">Ø {entry.avg_price} Taler</p>
                <p className="text-green-400">↓ {entry.low_price}</p>
                <p className="text-red-400">↑ {entry.high_price}</p>
                <p className="text-gray-400">{entry.shop_count} Shops</p>
              </div>
              <div
      ...
      <div className="flex justify-between text-[9px] text-gray-600 font-bold uppercase">
        <span>{history[0].recorded_at?.split('T')[0]}</span>
        <span>{history[history.length - 1].recorded_at?.split('T')[0]}</span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4 pt-4 border-t border-[#333]">
        <div className="text-center">
          <p className="text-green-400 font-bold text-sm flex items-center justify-center gap-1">
            {lastEntry.low_price} <TalerIcon className="w-3 h-3" />
          </p>
        ...
        <div className="text-center">
          <p className="text-yellow-500 font-bold text-sm flex items-center justify-center gap-1">
            {lastEntry.avg_price} <TalerIcon className="w-3 h-3" />
          </p>
        ...
        <div className="text-center">
          <p className="text-red-400 font-bold text-sm flex items-center justify-center gap-1">
            {lastEntry.high_price} <TalerIcon className="w-3 h-3" />
          </p>
