...
const ItemSearchPicker = ({ value, onChange, excludeIds = [], placeholder = 'Item suchen...' }: ItemSearchPickerProps) => {
  ...
  const results = search.trim().length > 0
    ? allItems.filter(i => !excludeIds.includes(i.id)).filter(i => i.name.toLowerCase().includes(search.toLowerCase())).slice(0, 8)
    : allItems.filter(i => !excludeIds.includes(i.id)).slice(0, 8);
  ...
  return (
    <div className="relative" ref={ref}>
      {selectedItem && !open ? (
        <button type="button" onClick={() => { setOpen(true); setSearch(''); }} className="mc-input bg-[#1a1a1a] w-full flex items-center gap-3 text-left cursor-pointer">
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-[#2a2a2a] border-2 border-[#1e1e1e] shadow-[0_10px_25px_rgba(0,0,0,0.5)] z-50 max-h-64 overflow-y-auto">
          {results.length > 0 ? (
            results.map(item => (
              <button key={item.id} type="button" onClick={() => selectItem(item)} className="flex items-center gap-3 p-2.5 w-full text-left hover:bg-[#323232] transition-colors border-b border-black/20">
                <div className="mc-item-slot bg-[#373737] w-8 h-8 shrink-0">
