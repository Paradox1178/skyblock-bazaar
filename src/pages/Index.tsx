...
import { ArrowRight, Flame, Sparkles, LayoutGrid, PlusSquare } from 'lucide-react';
...
const Index = () => {
  ...
  const popularItems = allItems.slice(0, 12);
  const newItems = [...allItems].slice(-12).reverse();
    ...
    <div className="min-h-screen bg-[#1a1a1a] text-white">
      <section className="mc-stone-panel border-b-4 border-black relative">
        ...
        <div className="container mx-auto px-4 py-16 text-center relative z-10">
          ...
          <p className="text-lg md:text-xl font-bold text-gray-200 max-w-2xl mx-auto mb-8 drop-shadow-[2px_2px_0px_#000]">
            ...
            {isLoading ? 'Lade Items...' : `Finde die günstigsten Preise unter ${allItems.length} Items.`}
          </p>
          <div className="flex flex-wrap gap-4 justify-center">
            <button onClick={() => navigate('/items')} className="mc-btn-primary px-8 py-4 text-sm font-pixel flex items-center gap-2">
              <LayoutGrid className="h-4 w-4" /> Items suchen
            </button>
            <button onClick={() => navigate('/submit')} className="mc-category-active px-8 py-4 text-sm font-bold flex items-center gap-2 border-white/20">
      ...
      <div className="container mx-auto px-4 py-12">
        <section className="mb-16">
          ...
          {isLoading ? (
            <p className="text-gray-400 animate-pulse text-center py-8">Lade Items...</p>
          ) : (
        ...
        <div className="mc-panel mt-20 p-8 text-center bg-[#2a2a2a]">
          ...
          <p className="text-gray-400 text-sm max-w-lg mx-auto mb-6">
            Hilf anderen Spielern, indem du deine Shop-Preise teilst. Je mehr Daten wir haben, desto besser für alle!
          </p>
