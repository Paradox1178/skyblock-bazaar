      ...
      <div className="container mx-auto px-4 py-8 max-w-xl text-left">
        ...
        <form onSubmit={handleSubmit} className="bg-[#2a2a2a] border-2 border-[#1e1e1e] shadow-[inset_3px_3px_0px_#3c3c3c] p-8 space-y-6">
          ...
          <div>
            ...
            >
              <option value="" disabled className="text-gray-600">-- Welches Item verkaufst du? --</option>
              {DEFAULT_ITEMS.sort((a, b) => a.name.localeCompare(b.name)).map(item => (
                <option key={item.id} value={item.id} className="bg-[#1a1a1a] text-white">
                  {item.name}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <textarea 
              className="mc-input bg-[#1a1a1a] min-h-[100px] resize-none" 
              value={form.description} 
              onChange={e => update('description', e.target.value)} 
              placeholder="Welche Kiste? Welcher Stock?" 
            />
          </div>
